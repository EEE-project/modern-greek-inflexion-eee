#!/usr/bin/env python3
"""
Test original library version (v2.0.7).

Reads TSV from stdin with verbs, tests with original library,
outputs results as TSV to stdout.

Usage (from original venv):
    source venv_original/bin/activate
    cat verbs.tsv | python3 test_original_lib.py > results_original.tsv
"""
...
def main():
    """Read verbs, test with original library, output results."""
    try:
        ...
        df['Aorist_Original'] = aorist_results
        df['Future_Original'] = future_results
